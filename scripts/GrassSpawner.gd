...
## 지정한 영역 안에 Grass 씬을 자동으로 뿌리는 스폰어.
## 던전 씬에 Node2D 를 만들고 이 스크립트를 붙여 사용.

const GRASS_SCENE := preload("res://AutoLoad/scenes/Grass.tscn")
...
func _draw() -> void:
	...
	draw_rect(rect, Color(0.15, 0.85, 0.15, 0.08), true)
	draw_rect(rect, Color(0.15, 0.85, 0.15, 0.7), false, 1.0)
	...
	for _i in count:
		var pos := _random_pos(rng)
		draw_circle(pos, 2.5, Color(0.1, 1.0, 0.1, 0.85))
