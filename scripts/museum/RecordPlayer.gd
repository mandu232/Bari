...
## 기록재생기 — 범위 내 전시대 에코의 활성도를 지속적으로 회복시키는 시설
## 전력이 공급될 때만 효과가 활성화됩니다

# ───────────────────────────────
#  EXPORT
# ───────────────────────────────
@export var activity_bonus: float = 11.0   ## 초당 활성도 회복량 (2.2/s 감소를 상쇄하고 서서히 채워줌)
@export var effect_range:   float = 110.0  ## 효과 반경 (픽셀)
...
func _draw() -> void:
	if not _show_range:
		return
	# 보라색 — 활성도·진동·음악 느낌
	draw_circle(Vector2.ZERO, effect_range, Color(0.75, 0.2, 1.0, 0.05))
	draw_arc(Vector2.ZERO, effect_range, 0.0, TAU, 72, Color(0.75, 0.2, 1.0, 0.6), 2.0)
...
func _process(delta: float) -> void:
	...
	for node in get_tree().get_nodes_in_group("placed_structure"):
		...
		if dist <= effect_range:
			slot.echo.needs.fulfill(&"활성도", activity_bonus * delta)
...
func _update_nearby_ui() -> void:
	if not is_powered and power_cost > 0:
		...
		_info_label.text = "🎵 활성도 +%.0f/s\n[F] 배선 연결" % activity_bonus
		_info_label.add_theme_color_override("font_color", Color(0.85, 0.5, 1.0))
