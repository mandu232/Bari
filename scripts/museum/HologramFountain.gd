extends Node2D
class_name HologramFountain
## 홀로그램 분수 — 범위 내 전시대의 영력 생산량을 증가시키는 시설
## 전력이 공급될 때만 효과가 활성화됩니다

# ───────────────────────────────
#  EXPORT
# ───────────────────────────────
@export var stability_bonus: float = 5.0   ## 초당 안정도 회복량 (1.0/s 감소를 상쇄하고 서서히 채워줌)
@export var effect_range:    float = 120.0 ## 효과 반경 (픽셀)
...
func _ready() -> void:
	...
	add_to_group("hologram_fountain")
...
func _draw() -> void:
	if not _show_range:
		return
	# 반투명 채우기 (청록색)
	draw_circle(Vector2.ZERO, effect_range, Color(0.15, 1.0, 0.75, 0.05))
	# 외곽 실선
	draw_arc(Vector2.ZERO, effect_range, 0.0, TAU, 72, Color(0.15, 1.0, 0.75, 0.6), 2.0)
...
func _process(delta: float) -> void:
	...
	for node in get_tree().get_nodes_in_group("placed_structure"):
		...
		if dist <= effect_range:
			slot.echo.needs.fulfill(&"안정도", stability_bonus * delta)
...
func _update_nearby_ui() -> void:
	if not is_powered and power_cost > 0:
		...
		_info_label.text = "💧 안정도 +%.0f/s\n[F] 배선 연결" % stability_bonus
		_info_label.add_theme_color_override("font_color", Color(0.15, 1.0, 0.75))
